...
let fbSleeping = localStorage.getItem('fb-sleeping') === '1';
...
function fbRemoveAllRunControls() {
    document.querySelectorAll('.fb-run-row, .fb-gemini-run-row, .fb-run-btn, button[title="Run in Terminal"]').forEach(el => el.remove());
}

function fbEnsureRunRows() {
    if (fbSleeping) {
        fbRemoveAllRunControls();
        return;
    }
...
const fbRunObserver = new MutationObserver(() => {
    ...
    fbRunDebounceTimer = setTimeout(fbEnsureRunRows, 250);
});
fbRunObserver.observe(document.body, { childList: true, subtree: true });
window.addEventListener('pageshow', fbEnsureRunRows);
window.addEventListener('focus', fbEnsureRunRows);
document.addEventListener('visibilitychange', () => {
    if (!document.hidden) fbEnsureRunRows();
});
setInterval(fbEnsureRunRows, 700);
fbEnsureRunRows();
setTimeout(fbEnsureRunRows, 800);
setTimeout(fbEnsureRunRows, 1800);
setTimeout(fbEnsureRunRows, 3200);
